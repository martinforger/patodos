-- ============================================================
-- Migración 11 · Agregar unidad_medida (opcional) a insumo
-- ------------------------------------------------------------
-- El campo es nullable: el nombre del insumo sigue siendo
-- la fuente principal de descripción, pero la unidad de medida
-- permite estandarizar consultas y reportes cuando se necesita.
-- ============================================================

-- ------------------------------------------------------------
-- 1. Agregar columna
-- ------------------------------------------------------------
ALTER TABLE insumo ADD COLUMN IF NOT EXISTS unidad_medida varchar(50) DEFAULT NULL;

-- ------------------------------------------------------------
-- 2. sp_crear_insumo  (con unidad_medida opcional)
-- SECURITY DEFINER: insumo es catálogo global sin política INSERT.
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS sp_crear_insumo(text, uuid);
...
BEGIN
  ...
  VALUES (
    p_categoria_id,
    trim(p_nombre),
    CASE WHEN trim(coalesce(p_unidad_medida, '')) = '' THEN NULL
  ...
  RETURN jsonb_build_object(
    ...
    'unidad_medida',  CASE WHEN trim(coalesce(p_unidad_medida, '')) = '' THEN NULL
                           ELSE trim(p_unidad_medida) END
  );
END;
$$;

REVOKE ALL ON FUNCTION sp_crear_insumo(text, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sp_crear_insumo(text, uuid, text) TO authenticated;

-- ------------------------------------------------------------
-- 3. sp_listar_insumos  (incluye unidad_medida)
-- ------------------------------------------------------------
